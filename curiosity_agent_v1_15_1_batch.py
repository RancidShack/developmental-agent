"""
curiosity_agent_v1_15_1_batch.py
----------------------------------
v1.15.1: Witnessed Navigation and the Socially Corroborated Prediction.

Implements the primary developmental question of the v1.15 version map
specification, which v1.15 did not answer:
  Does witnessed navigation inform an open prediction, and does social
  observation shorten the developmental arc?

ADDITIONS TO V1.15 ARCHITECTURE
  v1.15.1 adds four targeted components. The shared-world two-agent
  foundation (AgentCtx, cognitive independence, CRITICAL INVARIANT) is
  inherited unchanged.

  1. INTER-AGENT PERCEPTION
     Each agent perceives the other as an AGENT object when within
     PERCEPTION_RADIUS. Implemented via other_ctx parameter in _step_agent.

  2. SOCIAL OBSERVATION MECHANISM
     Fires in ENV2 when simultaneously: Agent B holds open prediction
     (state == predicted), Agent A is within CONTACT_RADIUS of att_blue,
     Agent B is within PERCEPTION_RADIUS of Agent A. On fire: predicted
     schema transitions to socially_corroborated, being_observed record
     written to Agent A, directed search bonus intensified for Agent B.

  3. SOCIALLY CORROBORATED SCHEMA STATE
     Fourth state: predicted -> socially_corroborated -> confirmed/unresolvable.
     A socially corroborated record is not a confirmation. Own mastery required.

  4. Q6 STATEMENT GENERATOR + BEING_OBSERVED RECORD + ZPD MEASUREMENT
     Q6: prediction step N, observation step M, location known, mastery required.
     being_observed: first second-person substrate in the programme.
     zpd_delta: Agent B arc_total_steps vs v1.14.1 naive baseline (230,734).

NEW OUTPUT FILES
  social_observation_v1_15_1.csv
  being_observed_v1_15_1.csv

ARCH = v1_15_1. Seeds from run_data_v1_15.csv. seed_b = seed_a + 20_000.

PRE-REGISTRATION: v1_15_1_pre_registration.md (8 May 2026).
"""
...
RUN_DATA_CSV             = "run_data_v1_15_1.csv"
PROVENANCE_CSV           = "provenance_v1_15_1.csv"
GOAL_CSV                 = "goal_v1_15_1.csv"
COUNTERFACTUAL_CSV       = "counterfactual_v1_15_1.csv"
BELIEF_REVISION_CSV      = "belief_revision_v1_15_1.csv"
CAUSAL_CSV               = "causal_v1_15_1.csv"
REPORT_CSV               = "report_v1_15_1.csv"
REPORT_SUMMARY_CSV       = "report_summary_v1_15_1.csv"
END_STATE_DRAW_CSV       = "end_state_draw_log_v1_15_1.csv"
ENV2_RUN_DATA_CSV        = "env2_run_data_v1_15_1.csv"
Q5_INDIVIDUATION_CSV     = "q5_individuation_v1_15_1.csv"
PREDICTED_SCHEMA_CSV     = "predicted_schema_v1_15_1.csv"
ARC_COMPLETE_CSV         = "arc_complete_v1_15_1.csv"
ARC_PAIRED_CSV           = "arc_paired_v1_15_1.csv"
SOCIAL_OBSERVATION_CSV   = "social_observation_v1_15_1.csv"   # v1.15.1
BEING_OBSERVED_CSV       = "being_observed_v1_15_1.csv"       # v1.15.1
...
def _check_social_observation(ctx_observer, ctx_observed, world, env_number):
    ...
    3. ctx_observer is within PERCEPTION_RADIUS of ctx_observed
    """
    ...
    # Condition 3: observer within PERCEPTION_RADIUS of observed
    dist = math.sqrt(
...
def main():
    ...
    try:
        ...
        print(f"v1.15.1 batch: {len(jobs)} runs x 2 agents = {len(jobs)*2} agent-runs")
        ...
        print()
        print("="*65)
        print(f"v1.15.1 complete. {len(jobs)-len(incomplete_runs)}/{len(jobs)} runs.")
        if report:
