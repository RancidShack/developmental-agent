"""
curiosity_agent_v1_5_batch.py
------------------------------
v1.5 prediction-error batch runner. Inherits the v1.4 architecture
unchanged; runs six parallel observers:
  1. v1.0 recorder           (V1Recorder)
  2. v1.1 provenance store   (V1ProvenanceStore)
  3. v1.2/v1.3 schema observer (V13SchemaObserver — 7 cell types)
  4. v1.3 family observer    (V13FamilyObserver)
  5. v1.4 comparison observer (V14ComparisonObserver)
  6. v1.5 prediction-error observer (V15PredictionErrorObserver)  ← new

Outputs:
  run_data_v1_5.csv              per-run metrics: all v1.4 fields
                                  retained plus v1.5 prediction-error
                                  summary fields.
  prediction_error_v1_5.csv     per-encounter prediction-error records.
                                  ← new output file this iteration
  provenance_v1_5.csv            per-flag formation records (v1.1 schema).
  snapshots_v1_5.csv             v1.1 provenance snapshots.
  schema_v1_5.csv                per-run schema content (7 cell types).
  family_v1_5.csv                per-run family record (v1.3 schema).
  comparison_v1_5.csv            per-run cross-family comparison record.
  snapshots_v1_0_under_v1_5.csv v1.0 snapshot file for continuity.

The v0.14 agent (via V15Agent subclass) is used unchanged. V13World is
inherited unchanged. The single architectural addition is
...
Flag combinations (pre-flight verification levels):
  ...
  --no-comparison --no-prediction-error:
      bit-for-bit identical to v0.14 baseline (level 1).
  --no-provenance --no-schema --no-family --no-comparison
  --no-prediction-error:
      matches v1.0 baseline (level 2).
  --no-schema --no-family --no-comparison --no-prediction-error:
      matches v1.1 baseline (level 3).
  --no-family --no-comparison --no-prediction-error:
      matches v1.2 baseline at matched seeds on all v1.2 metrics (level 4).
  --no-comparison --no-prediction-error:
      byte-identical to v1.3.2 baseline at matched seeds on all v1.3
      metrics. Permanent level-5 regression test (inherited from v1.4).
  --no-prediction-error only:
      byte-identical to v1.4 baseline at matched seeds on all v1.4
      metrics. This is the permanent level-6 regression test added to
      the pipeline by v1.5. See verify_v1_5_no_prediction_error.py.
  (defaults):
      full v1.5 production run with all six observers.

Seeds: matched to v1.4 batch via run_data_v1_4.csv.

Run from Terminal with:
    python3 curiosity_agent_v1_5_batch.py
"""
...
from v1_4_comparison_observer import (
    V14ComparisonObserver,
    COMPARISON_SUMMARY_FIELDS,
    COMPARISON_RECORD_FIELDS,
)
from v1_5_prediction_error_observer import (
    V15PredictionErrorObserver,
...
# Cell type constants dict — extended to include COLOUR_CELL
CELL_TYPE_CONSTANTS = {
    "FRAME":       FRAME,
    "NEUTRAL":     NEUTRAL,
    "HAZARD":      HAZARD,
...
def run_one(hazard_cost, num_steps, seed, run_idx,
            ...
            use_v13_world=True):
    """Execute one v1.5 run.

    record_prediction_error controls whether V15PredictionErrorObserver
    is instantiated. With record_prediction_error=False, output is
    byte-identical to v1.4 baseline at matched seeds (level-6 regression).

    The prediction-error observer requires record_family=True and
    use_v13_world=True; if either is False, record_prediction_error is
    treated as False.

    All flags False + use_v13_world False:
    ...
    instrument + provenance + schema + family + use_v13_world True,
    no comparison, no prediction-error:
        identical to v1.3.2 at matched seeds (level-5 regression).
    All active except prediction-error + use_v13_world True:
        identical to v1.4 at matched seeds (level-6 regression).
    All active + use_v13_world True:
        full v1.5 production run.
    """
    np.random.seed(seed)
    ...
    agent = V15Agent(world, num_steps)
    ...
    comparison_obs = None
    if record_comparison and family_obs is not None:
        comparison_obs = V14ComparisonObserver(
            family_observer=family_obs,
            run_metadata=run_meta,
        )

    # Prediction-error observer requires family observer and V13World.
    prediction_error_obs = None
    if record_prediction_error and family_obs is not None:
    ...
    for step in range(num_steps):
        ...
        if comparison_obs         is not None: comparison_obs.on_pre_action(step)
        if prediction_error_obs   is not None: prediction_error_obs.on_pre_action(step)
        ...
        if comparison_obs         is not None: comparison_obs.on_post_event(step)
        if prediction_error_obs   is not None: prediction_error_obs.on_post_event(step)
    ...
    if comparison_obs         is not None: comparison_obs.on_run_end(num_steps - 1)
    if prediction_error_obs   is not None: prediction_error_obs.on_run_end(num_steps - 1)

    # ------------------------------------------------------------------
    # Metrics assembly — all v1.4 fields retained, v1.5 fields appended.
    # ------------------------------------------------------------------
    metrics = {
        "arch":        "v1_5",
        "hazard_cost": hazard_cost,
    ...
    if comparison_obs is not None:
        metrics.update(comparison_obs.summary_metrics())
    else:
        for field in COMPARISON_SUMMARY_FIELDS:
            metrics[field] = None

    if prediction_error_obs is not None:
        metrics.update(prediction_error_obs.summary_metrics())
    else:
        for field in PREDICTION_ERROR_SUMMARY_FIELDS:
            metrics[field] = None

    return (metrics, recorder, provenance, schema_obs,
            family_obs, comparison_obs, prediction_error_obs)
...
def main():
    ...
    ap.add_argument("--out",                   default="run_data_v1_5.csv")
    ap.add_argument("--snapshots-out",         default="snapshots_v1_5.csv")
    ap.add_argument("--provenance-out",        default="provenance_v1_5.csv")
    ap.add_argument("--schema-out",            default="schema_v1_5.csv")
    ap.add_argument("--family-out",            default="family_v1_5.csv")
    ap.add_argument("--comparison-out",        default="comparison_v1_5.csv")
    ap.add_argument("--prediction-error-out",  default="prediction_error_v1_5.csv")
    ap.add_argument("--v10-snapshots-out",
                    default="snapshots_v1_0_under_v1_5.csv")
    ap.add_argument("--baseline",              default="run_data_v1_4.csv")
    ...
    ap.add_argument("--no-prediction-error",  action="store_true",
                    help=("Disable v1.5 prediction-error observer. With all "
                          "other observers active and V13World, output is "
                          "byte-identical to v1.4 baseline at matched seeds. "
                          "This is the permanent level-6 regression test "
                          "added by v1.5."))
    ...
    record_comparison       = not args.no_comparison
    record_prediction_error = not args.no_prediction_error
    ...
    for r in baseline:
        if r.get("arch") not in (
            "v1_4", "v1_3", "v1_2", "v1_1", "v1_0", "v0_14", "v0_14_repl"
        ):
            continue
        try:
            ...
            seed = int(r["seed"])
            priority = {
                "v1_4": 0, "v1_3": 1, "v1_2": 2, "v1_1": 3,
                "v1_0": 4, "v0_14": 5, "v0_14_repl": 6,
            }
    ...
    print(f"v1.5 PREDICTION-ERROR BATCH")
    print(f"  Architecture:              v0.14 (unchanged) + V13World + V15Agent")
    ...
    print(f"  v1.3 family:               {'ENABLED' if record_family else 'DISABLED'}")
    print(f"  v1.4 comparison:           {'ENABLED' if record_comparison else 'DISABLED'}")
    print(f"  v1.5 prediction-error:     {'ENABLED' if record_prediction_error else 'DISABLED'}")
    ...
    # Initialise output files (write headers unless appending)
    ...
    if record_comparison and not append_mode:
        with open(args.comparison_out, "w", newline="") as f:
            csv.DictWriter(f, fieldnames=COMPARISON_RECORD_FIELDS).writeheader()

    if record_prediction_error and not append_mode:
        with open(args.prediction_error_out, "w", newline="") as f:
            csv.DictWriter(
    ...
    comparison_complete_count    = 0
    prediction_error_complete_ct = 0
    total_pe_events              = 0
    t_start = time.time()

    for i, (cost, steps, run_idx, seed) in enumerate(jobs):
        t0 = time.time()
        (metrics, recorder, provenance, schema_obs, family_obs,
         comparison_obs, prediction_error_obs) = run_one(
            ...
            record_prediction_error=record_prediction_error,
        ...
        if family_obs is not None:
            ...
            family_obs.reset()
        if comparison_obs is not None:
            comparison_obs.write_comparison_csv(args.comparison_out, append=True)
            if metrics.get("cross_family_comparison_complete"):
                comparison_complete_count += 1
            comparison_obs.reset()
        if prediction_error_obs is not None:
            prediction_error_obs.write_prediction_error_csv(
                args.prediction_error_out, append=True
            )
            if metrics.get("prediction_error_complete"):
                prediction_error_complete_ct += 1
            total_pe_events += metrics.get("total_prediction_error_events") or 0
            prediction_error_obs.reset()
        ...
        # Prediction-error status for progress line
        y_pre = metrics.get("yellow_pre_transition_entries") or 0
        g_pre = metrics.get("green_pre_transition_entries") or 0
        y_rw  = metrics.get("yellow_resolution_window")
        g_rw  = metrics.get("green_resolution_window")
        rw_str = (
            f"Yrw={y_rw}" if y_rw is not None else "Yrw=---"
        ) + "/" + (
            f"Grw={g_rw}" if g_rw is not None else "Grw=---"
        )

        print(
            f"[{completed:>3}/{len(jobs)}] cost={cost} steps={steps} "
            f"run={run_idx}: mast={metrics['attractors_mastered']} "
            f"Ypre={y_pre} Gpre={g_pre} {rw_str} "
            f"({elapsed_run:.1f}s, ETA {eta/60:.1f}m)"
        )

    elapsed = time.time() - t_start
    print(f"\nv1.5 batch complete in {elapsed/60:.1f} minutes.")
    if instrument:
    ...
    if record_comparison:
        print(f"Comparison complete:             {comparison_complete_count} of {len(jobs)}")
    if record_prediction_error:
        print(f"Prediction-error complete runs:  {prediction_error_complete_ct} of {len(jobs)}")
        print(f"Total prediction-error events:   {total_pe_events}")

    # ------------------------------------------------------------------
    # Write per-run CSV — all v1.4 fields retained, v1.5 fields appended.
    # ------------------------------------------------------------------
    fieldnames = [
        ...
        "schema_complete",
    ] + FAMILY_SUMMARY_FIELDS + COMPARISON_SUMMARY_FIELDS \
      + PREDICTION_ERROR_SUMMARY_FIELDS
    ...
    if record_comparison:  print(f"v1.4 comparison:         {args.comparison_out}")
    if record_prediction_error:
        print(f"v1.5 prediction-error:   {args.prediction_error_out}")
