...
### 4.4 v1.15 in the programme arc: the first relational data

The v1.15 iteration is sometimes read, on a first pass through its results, as a ZPD experiment that failed to demonstrate ZPD. This reading is accurate as far as it goes, but it miscounts what the iteration produced.

The programme prior to v1.15 held only one relational category: the relationship between an agent and an object. Every schema record, every provenance entry, every causal chain is a record of an agent's encounter with something in its environment — a hazard, an attractor, a family pattern, a surprise. The agent is always the subject. The object is always the object. The relation is always agent-to-world.

v1.15 introduces a second relational category: the relationship between an agent and another agent. The social_observation record, the being_observed record, the symmetric observation data, and the mutual observation in run 11 are not failed ZPD measurements. They are the programme's first archive of agent-agent relational data. The 29 social observation events record, for the first time in the programme, one agent's cognitive response to another agent's behaviour at a shared object. The 29 being_observed records record, for the first time, one agent's developmental history including the fact of having been perceived by another agent at a moment of significance.

This distinction matters for the programme's forward trajectory. The relational grammar the agent builds through V1 is entirely agent-to-object: every schema record, every causal chain, every prediction is grounded in an encounter between one agent and one object. Any environment that introduces independently moving entities — entities with their own developmental histories that approach shared objects, occupy predicted locations, or respond to the agent's behaviour — will require a cognitive category that the V1 object-only grammar cannot supply. The agent will need a way to represent "another entity at the predicted location" that is richer than obstacle detection and anchored in something more than positional proximity.

The v1.15 substrate provides that category in embryonic form. The social_observation record carries: the observer's position, the observed agent's position, the shared object, the schema state before and after observation, and the step at which the observation fired. The being_observed record carries: the observed agent's position, the observer's identity, the shared object, and whether the observed agent had already mastered it. These are not the records of a failed experiment. They are the earliest entries in the programme's relational register for agent-agent encounters — the architectural precursor to recognising that some things in the world are not objects but subjects.

Run 11's mutual observation is the clearest statement of what was being built. At ENV2 step 63, Agent A and Agent B were simultaneously at att_blue, each in an open-prediction state, each observing the other, neither yet having mastered the object. Both being_observed records carry own_mastery_at_step=False. The programme has no prior event in which two agents were simultaneously aware of each other at a shared developmental object, before either had resolved it. That event is in the archive. Its significance in any environment where the agent encounters other agents with their own developmental trajectories is not yet measurable. What is measurable now is that the architecture can represent it and record it honestly.

The v1.15 data should be held as the programme's relational baseline: what agent-agent interaction looks like when both agents share a world and a developmental objective, before either has the cognitive architecture to use that interaction strategically. The ZPD question — whether witnessed navigation shortens the developmental arc — remains open, with its architectural path to resolution identified. But the prior question — whether the programme can represent and record agent-agent relational events at all — is answered. It can. The records are clean, the substrates resolve, and the data is in the archive.
