...
# ---------------------------------------------------------------------------
# Architecture unchanged from v1.13.4 except world fix (see v1_13_world.py)
# ---------------------------------------------------------------------------
...
RUN_DATA_CSV        = "run_data_v1_13_8.csv"
PROVENANCE_CSV      = "provenance_v1_13_8.csv"
GOAL_CSV            = "goal_v1_13_8.csv"
COUNTERFACTUAL_CSV  = "counterfactual_v1_13_8.csv"
BELIEF_REVISION_CSV = "belief_revision_v1_13_8.csv"
CAUSAL_CSV          = "causal_v1_13_8.csv"
REPORT_CSV          = "report_v1_13_8.csv"
REPORT_SUMMARY_CSV  = "report_summary_v1_13_8.csv"
END_STATE_DRAW_CSV  = "end_state_draw_log_v1_13_8.csv"
ENV2_RUN_DATA_CSV   = "env2_run_data_v1_13_8.csv"
Q5_INDIVIDUATION_CSV = "q5_individuation_v1_13_8.csv"
PREDICTED_SCHEMA_CSV = "predicted_schema_v1_13_8.csv"

ENV1_STEPS       = 1_000_000  # Environment 1 step budget — extended per Montessori
                              # principle: freedom to complete, not a ceiling.
                              # Early termination fires on arc completion so agents
                              # do not ruminiate past developmental completion.
ENV2_STEPS       =   800_000  # Environment 2 step budget — proportional extension.
...
RUN_DATA_FIELDS = [
    ...
    # v1.13.5 individuation signals
    "env1_mastery_order_sequence",   # ENV1 mastery order captured before carry_agent reset
    "env1_end_state_banked_step",    # step ENV1 end_state banked (honest arc length)
]
...
ENV2_RUN_DATA_FIELDS = [
    ...
    # v1.13.5 individuation signals — full ENV2 mastery arc
    "env2_mastery_order_sequence",     # order all ENV2 attractors were mastered
    "env2_att_yellow_mastery_step",    # step att_yellow mastered in ENV2
    "env2_att_green_mastery_step",     # step att_green mastered in ENV2
    "env2_actual_steps",               # true arc length in ENV2
]
...
def _check_transfer_condition(world, agent, predicted_record_written):
    """Return True when ENV1 Montessori completion gate is satisfied.

    v1.13.8: completion-based gate (not time-based phase machinery).

    Root cause of repeated gate failures (v1.13.4 through v1.13.7):
    V110Agent uses step-fraction phase transitions. Phase 3 (end_state draw)
    only activates at 60% of total_steps. With ENV1 at 1,000,000 steps,
    Phase 3 starts at step 600,000. The YELLOW+GREEN transfer fires at steps
    15k–772k — before Phase 3 in most runs — so end_state_banked is
    structurally unreachable under early termination.

    The Montessori principle is about completion of work, not completion of
    a time-based phase sequence. The room is done when the work is done.
    The gate therefore checks the completion conditions directly:

      1. All completable hazards knowledge-unlocked:
         every hazard in world.hazard_cells (which excludes haz_blue via
         unreachable_hazard_ids) has transitioned to KNOWLEDGE.
         This is the YELLOW + GREEN + unaffiliated triple confirmed.

      2. All attractors mastered:
         every attractor in world.attractor_cells has mastery_flag == 1.
         This confirms all free attractors and both family attractors.
         world.attractor_cells excludes the end_state cell.

      3. haz_blue approached — checked in outer guard before this call.

      4. Predicted schema record written for haz_blue — the agent carries
         the open question forward. This is what distinguishes v1.13 from
         v1.12: the transfer credential is not just completion but inference.
    """
    # Condition 1: all completable hazards knowledge-unlocked
    if not all(world.knowledge_unlocked.get(h, False)
               for h in world.hazard_cells):
        return False

    # Condition 2: all attractors mastered
    mastery_flag = getattr(agent, 'mastery_flag', {})
    if not all(mastery_flag.get(a, 0) == 1
               for a in world.attractor_cells):
        return False

    # Condition 3: predicted record written for haz_blue
...
def _run_environment(
    ...
    # v1.13.4: haz_blue declared unreachable in ENV1 so it is excluded from
    # the all_hazards_banked gate. This allows the end_state to fire once all
    # completable work is done (YELLOW + GREEN + unaffiliated hazards + free
    # attractors). The Montessori principle is restored: the agent must finish
    # the room before the door to ENV2 opens. haz_blue is still placed in
    # the world, approached in Phase 1, and triggers the predicted record —
    # it is simply not a completion requirement because it cannot be completed.
    ...
    if carry_agent is not None:
        ...
        # v1.13.4: Replace mastery_flag entirely rather than iterating over
        # world.attractor_cells. The old approach left historical ENV1 entries
        # (e.g. att_free_3 if consumed as end_state in ENV2) at 1, causing
        # sum(mastery_flag.values()) == len(world.attractor_cells) to fire
        # accidentally at ENV2 step ~286 on carried state.
        # Full replacement guarantees the sum is 0 on ENV2 entry.
        # att_blue is included because it is in world.attractor_cells for ENV2.
        # No Q-value prior — att_blue is encountered genuinely in Phase 1.
        if hasattr(agent, 'mastery_flag'):
            agent.mastery_flag = {a: 0 for a in world.attractor_cells}

        # v1.13.4: Replace knowledge_banked entirely for the same reason.
        # The old approach left ENV1 hazard entries (absent from ENV2 world)
        # at True, allowing the all_hazards_banked gate to fire immediately.
        ...
        # v1.13.6: remove unreachable hazards from agent's knowledge_banked.
        #
        # V110Agent initialises knowledge_banked from world.object_type rather
        # than world.hazard_cells, so haz_blue (HAZARD type in object_type but
        # excluded from hazard_cells by the unreachable_hazards world fix) is
        # included as a key with value False. The all_hazards_banked gate then
        # compares sum(True values) against len(knowledge_banked) or similar,
        # producing 5 == 6 which is permanently False — activation never fires.
        #
        # Removing unreachable hazard ids from knowledge_banked aligns the
        # agent's internal gate with world.hazard_cells. The Montessori
        # principle: unreachable hazards are structural facts about the current
        # environment, not completion requirements. The room is finished when
        # all completable work is done; haz_blue cannot be completed in ENV1
        # by architectural design, not by agent failure.
    ...
    att_yellow_mastery_step = None  # v1.13.5 individuation
    att_green_mastery_step  = None  # v1.13.5 individuation
    transfer_condition_met = False
    actual_steps = num_steps  # updated on early exit
    ...
    for step in range(num_steps):
        ...
        # v1.13.4 fix: attractor cells do not transform to KNOWLEDGE type —
        # only hazard cells do. The previous check (world.object_type == KNOWLEDGE)
        # could never fire for att_blue. Correct check is mastery_flag.
        if (env_number == 2 and att_blue_mastery_step is None
                and contact_oid == "att_blue"):
            if getattr(agent, 'mastery_flag', {}).get('att_blue', 0) == 1:
        ...
        # ---- att_yellow / att_green mastery step tracking (ENV2 only) ----
        # v1.13.5 individuation: captures when each family attractor is mastered
        # in ENV2 so the order and timing of the full arc can be compared across
        # runs. Combined with att_blue_mastery_step and att_blue_sequence_position
        # this gives the complete ENV2 developmental sequence per agent.
        if env_number == 2:
        ...
        if (env_number == 1 and not transfer_condition_met
                and haz_blue_approached):
            if with_prediction and schema_v13_obs is not None:
                if _check_transfer_condition(world, agent,
                                             predicted_record_written):
                    transfer_condition_met = True
                    transfer_triggered_step = step
            else:
                # --no-prediction: same completion gate, no predicted record req
                if _check_transfer_condition(world, agent, True):
                    transfer_condition_met = True
                    transfer_triggered_step = step

        # ---- Early termination: arc complete, prevent post-completion rumination ----
        # ENV1: exits as soon as transfer condition is met. The agent has done
        #       everything this environment offers; continuing only generates noise.
        # ENV2: exits as soon as end_state is banked. Same principle.
        # actual_steps records the true arc length for honest reporting.
        # Agents that never complete run to full budget (genuine developmental data).
        if env_number == 1 and transfer_condition_met:
            actual_steps = step + 1
            break
        if env_number == 2 and end_state_banked_step is not None:
            actual_steps = step + 1
            break
    ...
    return dict(
        ...
        att_yellow_mastery_step=att_yellow_mastery_step,  # v1.13.5
        att_green_mastery_step=att_green_mastery_step,    # v1.13.5
        actual_steps=actual_steps,  # true arc length; <= num_steps on early exit
        meta=meta,
        world=world,
    )
...
def run_one(run_idx, seed, hazard_cost,
    ...
    # v1.13.5: capture ENV1 mastery order BEFORE carry_agent resets it.
    # For transferring runs the carry_agent block clears mastery_order_sequence.
    # Snapshotting here preserves the true ENV1 developmental arc for individuation.
...
def _build_run_row(run_idx, seed, hazard_cost, env1,
    ...
    return {
        ...
        **goal_fields,
        ...
        # v1.13.5 individuation — ENV1 arc
...
def _build_env2_row(run_idx, seed, hazard_cost, env2_seed,
    ...
    # v1.13.4: att_blue_sequence_position computed from mastery_order_sequence.
    # Position 0 = mastered first; higher = mastered after familiar families.
    # This is the individuation signal: does the agent prioritise its directed
    # search target or complete familiar families first?
    ...
    return {
        ...
        # v1.13.5 individuation — full ENV2 developmental arc
        "env2_mastery_order_sequence":    str(getattr(agent, 'mastery_order_sequence', [])),
