"""
curiosity_agent_v1_15_2_batch.py
----------------------------------
v1.15.2: Witnessed Navigation — corrected social observation geometry and
Q6 reporting. Inherits the complete v1.15.1 architecture unchanged except
for two targeted fixes identified in the v1.15.1 post-run analysis.

FIXES FROM V1.15.1
  1. SOCIAL OBSERVATION GEOMETRY (critical)
     v1.15.1 used PERCEPTION_RADIUS (3.0) for condition 3 of the social
     observation gate. Combined with CONTACT_RADIUS (0.8) in condition 2,
     Agent B was at most 3.8 units from att_blue when the observation fired
     — confirmed within a mean of 5 steps regardless of the directed search
     bonus. arc_total_steps_b was bit-for-bit identical to v1.15 across all
     13 soc=S runs. The ZPD hypothesis was architecturally present but
     geometrically untestable.
     Fix: SOCIAL_PERCEPTION_RADIUS = 10.0. Used only in condition 3 of
     _check_social_observation. PERCEPTION_RADIUS (3.0) unchanged elsewhere.
     Agent B now observes Agent A at att_blue from up to ~10.8 units away,
     giving the 5x directed search bonus real distance to work on.

  2. Q6 STATEMENT TYPE (reporting bug)
     _generate_q6_statements returned plain dict objects. The report writer
     and summary use getattr(s, "query_type", "") which returns "" on a dict.
     Statements were appended to stmts_list but silently dropped to empty
     strings in every CSV write. statements_q6 = 0 for all 80 rows despite
     25 social corroboration events.
     Fix: SimpleNamespace objects. getattr works correctly on SimpleNamespace.

ARCHITECTURE (inherited unchanged from v1.15.1)
  1. INTER-AGENT PERCEPTION via other_ctx parameter in _step_agent.
  2. SOCIAL OBSERVATION MECHANISM — fires in ENV2 on three conditions.
  3. SOCIALLY CORROBORATED SCHEMA STATE — fourth state, not a confirmation.
  4. Q6 STATEMENT GENERATOR + BEING_OBSERVED RECORD + ZPD MEASUREMENT.

NEW OUTPUT FILES
  social_observation_v1_15_2.csv
  being_observed_v1_15_2.csv

ARCH = v1_15_2. Seeds from run_data_v1_15.csv. seed_b = seed_a + 20_000.

PRE-REGISTRATION: v1_15_1_pre_registration.md (8 May 2026) — v1.15.2 runs
under the same pre-registered hypothesis. Two fixes recorded as amendments.
"""
...
RUN_DATA_CSV             = "run_data_v1_15_2.csv"
PROVENANCE_CSV           = "provenance_v1_15_2.csv"
GOAL_CSV                 = "goal_v1_15_2.csv"
COUNTERFACTUAL_CSV       = "counterfactual_v1_15_2.csv"
BELIEF_REVISION_CSV      = "belief_revision_v1_15_2.csv"
CAUSAL_CSV               = "causal_v1_15_2.csv"
REPORT_CSV               = "report_v1_15_2.csv"
REPORT_SUMMARY_CSV       = "report_summary_v1_15_2.csv"
END_STATE_DRAW_CSV       = "end_state_draw_log_v1_15_2.csv"
ENV2_RUN_DATA_CSV        = "env2_run_data_v1_15_2.csv"
Q5_INDIVIDUATION_CSV     = "q5_individuation_v1_15_2.csv"
PREDICTED_SCHEMA_CSV     = "predicted_schema_v1_15_2.csv"
ARC_COMPLETE_CSV         = "arc_complete_v1_15_2.csv"
ARC_PAIRED_CSV           = "arc_paired_v1_15_2.csv"
SOCIAL_OBSERVATION_CSV   = "social_observation_v1_15_2.csv"   # v1.15.1
BEING_OBSERVED_CSV       = "being_observed_v1_15_2.csv"       # v1.15.1
...
def _fire_social_observation(ctx_b, ctx_a, world, step, env_number):
    """Execute social observation: update schema, write substrates."""
    # Update schema state
...
def main():
    ...
    try:
        ...
        print(f"v1.15.2 batch: {len(jobs)} runs x 2 agents = {len(jobs)*2} agent-runs")
        ...
        print()
        print("="*65)
        print(f"v1.15.2 complete. {len(jobs)-len(incomplete_runs)}/{len(jobs)} runs.")
        if report:
