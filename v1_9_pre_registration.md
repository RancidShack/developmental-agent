...
v1.9 advances the SICC trajectory on two commitments and positions a third. Underlying all three is the programme's Montessori framing: the environment is prepared so that contact with objects, in the agent's own sequence and at the agent's own tempo, produces the developmental event. The child does not need instruction; the materials are arranged so that the route through them becomes the child's own. What the child later carries away is not only the competencies acquired but the route by which they were acquired — the recollection of what came before what, which approaches succeeded and which did not, where they held back and what they knew when they held back. That route is part of what the child knows about themselves. The programme is building the substrate for this: self-knowledge emerging not from a self-model bolted on as a primary module, but from the accumulated record of object engagement, in the sequence and manner the agent chose. v1.9 is the iteration in which the register first records not just what was encountered and what was learned, but what was approached and refused. That addition is the substrate's recognition that restraint, like mastery, is part of the developmental arc the agent will eventually be able to give an account of.

**Commitment 3 (unfillable slots as epistemic features)** is advanced at the behavioural level. Prior iterations have held unfillable slots as structural absences — things the agent's body cannot measure. v1.9 extends this to behavioural epistemics: a suppressed approach is a slot the agent *could* have filled but did not. The counterfactual record holds the attempted access and the withdrawal. This is the first iteration in which the architecture records not only what the agent knows and does not know, but what it approached knowing and chose not to complete. In Montessori terms: the child who approached the material and turned away is having a developmental event that the environment should hold. The agent's register now holds it.
...
**Commitment 8 (self-knowledge as derivative of object-knowledge)** is advanced directly. The SICC document states that the agent comes to know itself through patterns in how it engages with what is not itself — that self-modelling built as a primary capacity produces hollow self-reports, whereas self-knowledge built as derivative produces grounded ones because the report can always be traced back to the object-encounters that gave rise to it. The suppressed-approach record is an object-engagement record of a new kind: the engagement that did not complete. The agent that can later report *I approached haz_yellow and withdrew, before I had mastered att_yellow* is producing a self-account grounded in an object encounter — the approach trajectory and the withdrawal are both records of the agent's relationship with a specific object at a specific point in its developmental arc. The biographical register is not growing because the agent is being given more self-model; it is growing because its object-engagement record is becoming richer, and self-knowledge follows.
