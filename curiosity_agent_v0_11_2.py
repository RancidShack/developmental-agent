"""
curiosity_agent_v0_11_2.py
--------------------------
Developmental agent in a 20x20 structured environment — v0.11.2
amendment to v0.11.1, blocking preference accumulation entirely on
mastered cells.

The v0.11 architecture introduced attractor depletion (feature_reward
goes to zero post-mastery) and primitive attraction bias clearing for
mastered attractors. The v0.11 probe revealed that cell_preference
persisted on mastered cells, dominating Phase 3 action selection.

The v0.11.1 amendment added a reset of cell_preference at the banking
step. The v0.11.1 probe revealed that this addressed only the
banking-time deposit; ongoing preference accumulation from
learning-progress reward continued to compound on mastered cells
across post-banking visits, eventually reconstructing the fixation
pattern (one mastered attractor visited 944 times in the v0.11.1 probe).

v0.11.2 completes the architectural intent: preference accumulation
in update_model is blocked entirely on mastered cells. Combined with
the v0.11 changes (depleted feature reward, cleared attraction bias)
and the v0.11.1 reset, this means a banked attractor produces no
signal pulling the agent back via any of the four mechanisms:

  1. Feature reward: zero post-mastery (v0.11)
  2. Primitive attraction bias: cleared (v0.11)
  3. Cell preference at banking: reset to zero (v0.11.1)
  4. Cell preference accumulation post-banking: blocked (v0.11.2)

The mastered cell becomes inert with respect to all preference and
feature signals. The cell remains visitable but no architectural
mechanism reinforces returning to it.

See v0.11.2-preregistration-amendment.md for full reasoning, including
the stopping rule named in Section 7 of that document.

All other architectural elements from v0.11/v0.11.1 (which inherit
v0.10) are retained unchanged.

Run from Terminal with:
    python3 curiosity_agent_v0_11_2.py
"""
...
def run():
    ...
    report_filename = f"self_report_v0_11_2_c{HAZARD_COST}.txt"
    ...
    plot_filename = f"run_output_v0_11_2_c{HAZARD_COST}.png"
