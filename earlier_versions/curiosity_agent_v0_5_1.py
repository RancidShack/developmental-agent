"""
curiosity_agent_v0_5_1.py
-------------------------
Refinement of v0.5: the interpretation layer is now properly
evidence-weighted. The agent says less, but what it says is true.

Version 0.5.1 — changes from v0.5:
  * THRESHOLDS NORMALISED. All claims now compare observed values to
    null-model expectations (uniform attention, random movement) rather
    than arbitrary integer thresholds. A claim fires only if the
    observed value departs meaningfully from what would happen by chance.
  * SILENCE RULE. If a feature does not pass its evidence threshold,
    the agent says nothing about it. No filler, no padded hedges.
  * EVIDENCE SUMMARY. The structured report now lists each potential
    claim and whether it fired (and why or why not), so we can see what
    the agent considered saying and chose not to.
  * NARRATIVE may be much shorter than v0.5. This is correct behaviour.
    Sparse honesty is preferred over fluent vagueness.

Run from Terminal with:
    python3 curiosity_agent_v0_5_1.py
"""
...
# --------------------------------------------------------------------------
# 3. THE INTERPRETATION LAYER (v0.5.1 — evidence-weighted)
# --------------------------------------------------------------------------

def interpret_self(agent, coverage_complete_step):
    """
    Compute features and produce evidence-weighted claims.
    Each potential claim is evaluated against an evidence threshold.
    Claims that don't pass are silenced and recorded in the evidence log.
    """
    log = agent.self_log
    n_steps = len(log)
    ...
    features = {}
    evidence_log = []   # records every claim considered, fired or not
    claims = []          # only the claims that fire
    ...
    # Evidence thresholds: the gap between top and runner-up
    if drive_gap >= 0.20:
        strength = "strongly"
        fire = True
    elif drive_gap >= 0.10:
    ...
    if fire:
        ...
        evidence_log.append(
            f"Drive tendency: silent — gap only {100*drive_gap:.1f}pp "
            f"(needs >= 5pp to fire)"
        )

    # ----------------- REGION AFFINITY (post-coverage only) -----------------
    if coverage_complete_step is not None and coverage_complete_step < n_steps - 500:
        post = log[coverage_complete_step:]
        cell_counts_post = Counter(r["cell"] for r in post)
        post_total = sum(cell_counts_post.values())
        n_cells_post = len(cell_counts_post)
        uniform_expected = post_total / n_cells_post
        most_visited = cell_counts_post.most_common(3)
        top_cell, top_n = most_visited[0]
        ratio_vs_uniform = top_n / uniform_expected
        # The claim only fires if the top cell is at least 2x uniform
        if ratio_vs_uniform >= 3.0:
            claims.append(
                f"After I had covered the map, I returned repeatedly to cell {top_cell}, "
                f"visiting it {ratio_vs_uniform:.1f} times more often than uniform attention "
                f"would have produced."
                + (f" This was a feature cell." if top_cell in FEATURE_CELLS else "")
            )
            evidence_log.append(
                f"Region affinity: FIRED (strong) — top cell {ratio_vs_uniform:.1f}x uniform"
            )
        elif ratio_vs_uniform >= 2.0:
            claims.append(
                f"After coverage was complete, I showed a mild preference for cell {top_cell}, "
                f"visiting it {ratio_vs_uniform:.1f} times more often than uniform attention "
                f"would predict."
                + (f" This was a feature cell." if top_cell in FEATURE_CELLS else "")
            )
            evidence_log.append(
                f"Region affinity: FIRED (mild) — top cell {ratio_vs_uniform:.1f}x uniform"
            )
        else:
            evidence_log.append(
                f"Region affinity: silent — top cell only {ratio_vs_uniform:.1f}x uniform "
                f"(needs >= 2.0 to fire)"
            )
    else:
        evidence_log.append("Region affinity: not evaluated — coverage not complete or run too short post-coverage")
    ...
    max_persistence = max(runs)
    features["mean_persistence"] = mean_persistence
    features["max_persistence"] = max_persistence
    # Null model: under random movement in a 10x10 grid, mean persistence
    # is approximately 1.0 (very rare to step in place because all 4 actions
    # take you elsewhere; only walls cause "stay put").
    # Real lingering requires evidence above this baseline.
    if mean_persistence >= 1.5:
        claims.append(
            f"I tended to linger in cells — my mean persistence was {mean_persistence:.2f} "
            f"steps per cell, well above the ~1.0 baseline expected from random movement."
        )
        evidence_log.append(
            f"Persistence: FIRED (lingering) — mean {mean_persistence:.2f} > 1.5 threshold"
        )
    elif mean_persistence <= 1.02:
        claims.append(
            f"I almost never lingered — my mean persistence was {mean_persistence:.2f} "
            f"steps per cell, essentially at the random-movement baseline."
        )
        evidence_log.append(
            f"Persistence: FIRED (no lingering) — mean {mean_persistence:.2f} <= 1.02"
        )
    else:
        evidence_log.append(
            f"Persistence: silent — mean {mean_persistence:.2f} is unremarkable "
            f"(near 1.0 baseline)"
        )

    # ----------------- ATTENTION DISTRIBUTION (replaces 'abandonment') -----------------
    ...
    # Coefficient of variation as a measure of how uneven attention was
    cv = np.std(visits) / np.mean(visits)
    features["attention_cv"] = cv
    features["max_visit"] = max(visits)
    features["min_visit"] = min(visits)
    features["max_over_uniform"] = max(visits) / expected_per_cell
    # Uniform attention has CV = 0; pathological corner-camping CVs near 1+
    if cv >= 0.6:
        claims.append(
            ...
            f"and the spread across cells was wide."
        )
        evidence_log.append(f"Attention distribution: FIRED (uneven) — CV {cv:.2f} >= 0.6")
    elif cv <= 0.3:
        claims.append(
            f"My attention was distributed fairly evenly across the cells I visited "
            f"(coefficient of variation {cv:.2f})."
        )
        evidence_log.append(f"Attention distribution: FIRED (even) — CV {cv:.2f} <= 0.3")
    else:
        evidence_log.append(
            f"Attention distribution: silent — CV {cv:.2f} is moderate, neither very "
            f"even nor very skewed"
        )
    ...
    if feature_ratio >= 1.4:
        claims.append(
            f"Cells with distinct observations drew me disproportionately — I spent "
            f"{feature_ratio:.2f} times more time in them than chance would predict."
        )
        ...
        claims.append(
            ...
            f"only {feature_ratio:.2f} times the rate chance would predict."
        )
        evidence_log.append(f"Feature affinity: FIRED (avoidance) — ratio {feature_ratio:.2f}")
    else:
        evidence_log.append(
            f"Feature affinity: silent — ratio {feature_ratio:.2f} is near chance"
        )

    return features, claims, evidence_log


def format_structured_report(features, agent, coverage_complete_step, evidence_log):
    lines = []
    lines.append("=" * 60)
    lines.append("STRUCTURED CHARACTERISATION (v0.5.1)")
    lines.append("=" * 60)
    ...
    lines.append("")
    lines.append("Drive profile (% of steps where this drive dominated):")
    for k, v in sorted(features["drive_shares"].items(), key=lambda kv: -kv[1]):
        lines.append(f"  {k:10s}: {100*v:5.1f}%")
    lines.append("")
    lines.append(f"Persistence  : mean {features['mean_persistence']:.2f}, "
                 f"max {features['max_persistence']}")
    lines.append(f"Attention CV : {features['attention_cv']:.2f}  "
                 f"(0 = perfectly even, >0.6 = very uneven)")
    lines.append(f"Max-visit cell over uniform: {features['max_over_uniform']:.1f}x")
    lines.append(f"Feature cell visit ratio   : {features['feature_ratio']:.2f}x chance")
    lines.append("")
    lines.append("EVIDENCE LOG (each potential claim and whether it fired):")
...
def format_narrative_report(claims):
    lines = []
    lines.append("=" * 60)
    lines.append("NARRATIVE CHARACTERISATION (v0.5.1)")
    lines.append("(the agent describing itself — claims grounded in evidence)")
    lines.append("=" * 60)
    lines.append("")
    if not claims:
        lines.append("I find I have little to say about myself with confidence.")
        lines.append("My behaviour did not depart meaningfully from chance in any of")
        lines.append("the dimensions I am able to examine.")
    else:
        for c in claims:
            lines.append(c)
            lines.append("")
...
def run():
    ...
    plt.tight_layout()
    plt.savefig("run_output_v0_5_1.png", dpi=120)

    print("\n")
    print(format_structured_report(features, agent, coverage_complete_step, evidence_log))
    print("\n")
    print(format_narrative_report(claims))

    with open("self_report_v0_5_1.txt", "w") as f:
        f.write(format_structured_report(features, agent, coverage_complete_step, evidence_log))
        f.write("\n\n")
        f.write(format_narrative_report(claims))

    print("\nReports also saved to self_report_v0_5_1.txt")
    print("Plots saved to run_output_v0_5_1.png")
