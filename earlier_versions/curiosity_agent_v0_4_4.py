"""
curiosity_agent_v0_4_4.py
-------------------------
Phase-structured developmental agent with prescribed traversal in Phase 1.

Branches from v0.4.3 after v0.4.3 again failed to reliably complete
coverage even with novelty active as a compass. The diagnosis: the
agent's choice-making apparatus is itself the source of exploration
bias, whatever drives feed it. Removing the choosing in Phase 1 —
prescribing movement — removes the bias.

This maps onto a real pedagogical principle. A child in a structured
space is free to choose activities, but not free in how they move
between them. Procedural rules — walk, don't climb, don't disturb —
precede the activity choices. The learning about the space accumulates
as a side effect of following the movement protocol.

PHASE 1 — ACQUISITION (prescribed)
    The agent traverses the grid in a boustrophedon (snake) pattern:
    row 0 left-to-right, row 1 right-to-left, row 2 left-to-right, etc.
    No choice, no drives active. The agent updates its forward model,
    visit counts, and error windows as it moves. Duration: exactly 100
    steps for a 10x10 grid.

PHASE 2 — INTEGRATION
    Choice returns. Novelty + Progress drives active.
    The agent now has a complete forward model and visit counts of
    every cell — a clean, uniform substrate.

PHASE 3 — AUTONOMY
    Novelty + Progress + Preference.

MEMORY ACROSS PHASES:
  - Forward model, error windows, visit counts, preferences: carry over.
  - Q-values: partially reset (× 0.3) at phase transitions.

Run from Terminal with:
    python3 curiosity_agent_v0_4_4.py
"""
...
class GridWorld:
    ...
    def set_position(self, pos):
        """Used only by prescribed Phase 1 traversal."""
        self.agent_pos = pos
...
def boustrophedon_path(size):
    """
    Generate the sequence of cells for a boustrophedon (snake) traversal.
    Row 0: left to right. Row 1: right to left. Alternating.
...
def action_to_move(from_cell, to_cell):
    """Compute which discrete action moves from_cell to to_cell."""
    fx, fy = from_cell
    tx, ty = to_cell
    dx = tx - fx
    dy = ty - fy
    ...
    return None  # same cell or non-adjacent — shouldn't happen in boustrophedon
...
class PhasedAgent:
    def __init__(self, scope_cells, total_steps, num_actions=4):
        ...
        self.path_index = 0  # index into prescribed_path
    ...
    def _apply_phase_weights(self):
        if self.phase == 1:
            # Phase 1 drives are silent — movement is prescribed, not chosen.
            # Coverage discovery still records as we move, but it doesn't
            # influence behaviour because we're not using Q-values here.
    ...
    def get_prescribed_action(self, current_cell):
        """
        Phase 1 movement: determine the next action from the prescribed path.
        Returns the action needed to move from current_cell to the next
        cell on the path.
        """
        if self.path_index == 0:
            # At the start — if we're not at path[0], we need to get there.
            # In the boustrophedon starting at (0,0), and world starts at (0,0),
            # we're already there. Advance the index.
            self.path_index = 1
        if self.path_index >= len(self.prescribed_path):
    ...
    def choose_action(self, state):
        """Drive-based choice, used only in Phase 2 and 3."""
    ...
    def update_values(self, state, action, next_state, intrinsic):
        # Only update Q-values in Phase 2 and 3
        if self.phase == 1:
            return
...
def run():
    ...
    for step in range(NUM_STEPS):
        ...
        # Action selection depends on phase
        if agent.phase == 1:
            current_cell = (state[0], state[1])
            action = agent.get_prescribed_action(current_cell)
            if action is None:
                # Path is complete; transition should handle on next iteration.
                # Take a neutral action to advance step counter.
                action = 0
        else:
    ...
    if p3_total_visits > 0:
        p3_concentration = p3_visits.max() / (p3_total_visits / (GRID_SIZE * GRID_SIZE))
    else:
    ...
    ax.set_title(f"Phase 1 (Prescribed traversal)\n"
                 f"ended at step {agent.phase_1_end_step}")
    ...
    plt.tight_layout()
    plt.savefig("run_output_v0_4_4.png", dpi=120)

    # --- REPORT ---
    lines = []
    lines.append("=" * 68)
    lines.append("STRUCTURED CHARACTERISATION (v0.4.4 — prescribed Phase 1)")
    lines.append("=" * 68)
    lines.append(f"Life length              : {NUM_STEPS} steps")
    lines.append("")
    lines.append("Phase 1 (prescribed boustrophedon traversal):")
    if agent.phase_1_end_step is not None:
        lines.append(f"  Completed at step      : {agent.phase_1_end_step}")
        lines.append(f"  Coverage at end of P1  : {len([c for c in agent.covered])}/100")
        ...
        lines.append(f"  Not completed — reached {len(agent.covered)}/100")
    lines.append("")
    ...
    if agent.phase_1_end_step:
        lines.append(f"My first {agent.phase_1_end_step} steps were not mine to shape. "
                     f"I walked the grid in the way I was told — row by row, back and forth, "
                     f"without choosing where to go or what to notice. I learned the shape of "
                     f"my world from this walking. My preferences, had I been allowed any, "
                     f"would have been silent.")
        lines.append("")
    if agent.phase_1_end_step and agent.phase_2_end_step:
        lines.append(f"I then entered integration. Novelty and progress drove me through "
                     f"{agent.phase_2_end_step - agent.phase_1_end_step} steps. I returned "
                     f"to the territory I had walked, now attending to what pulled me and "
                     f"what I was learning. The map I had was even, without bias — my "
                     f"preferences could form against any region.")
        lines.append("")
    if agent.phase_2_end_step:
        lines.append(f"At step {agent.phase_2_end_step} I entered autonomy. The regions I "
                     f"had come to find rewarding could now pull me back. Whatever I did "
                     f"with my final time reflects what I had become.")

    report_str = "\n".join(lines)
    print("\n" + report_str)
    with open("self_report_v0_4_4.txt", "w") as f:
        f.write(report_str)
    print("\nReport saved to self_report_v0_4_4.txt")
    print("Plots saved to run_output_v0_4_4.png")
    plt.show()
