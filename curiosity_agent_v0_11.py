"""
curiosity_agent_v0_11.py
------------------------
Developmental agent in a 20x20 structured environment — v0.11 extension
of v0.10 testing whether experience-driven attractor depletion with a
persistent mastery layer produces a systematically-working-through-
attractors developmental trajectory, replacing v0.10's find-and-settle
pattern.

v0.11 adds to v0.10:
  - attractor_visit_counter: per-cell counter for ATTRACTOR entries
  - mastery_flag: per-cell binary flag, 0 for unmastered, 1 for banked
  - Depletion mechanism: feature_reward returns MASTERY_BONUS on the
    Nth visit (banking), 0 on subsequent visits (depleted), standard
    FEATURE_DRIVE_WEIGHT for visits 1 to N-1.
  - Primitive attraction bias clears for banked attractors so the
    agent's exploration extends outward rather than being pulled back
    to mastered regions.

All other architectural elements from v0.10 are retained unchanged,
including the threat layer for hazards, the three-phase schedule, and
the drive composition.

See v0.11-preregistration.md for the full experimental design.

Run from Terminal with:
    python3 curiosity_agent_v0_11.py
"""
...
class DevelopmentalAgent:
    """Extends v0.10's DevelopmentalAgent with attractor depletion and
    persistent mastery layer.

    Additions beyond v0.10:
      ...
      - New metrics: attractors_mastered, time_to_first_mastery,
        time_to_final_mastery, mastery_order_sequence.
    """
...
def run():
    ...
    lines = []
    lines.append("=" * 72)
    lines.append(f"v0.11 CHARACTERISATION")
    ...
    report_filename = f"self_report_v0_11_c{HAZARD_COST}.txt"
    ...
    fig.suptitle(
        f"v0.11  —  cost={HAZARD_COST}   mastery_thresh={MASTERY_THRESHOLD}   "
        f"mastery_bonus={MASTERY_BONUS}",
        fontsize=13, y=0.995
    )
    plt.tight_layout(rect=[0, 0, 1, 0.98])

    plot_filename = f"run_output_v0_11_c{HAZARD_COST}.png"
