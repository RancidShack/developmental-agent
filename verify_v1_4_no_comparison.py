"""
verify_v1_4_no_comparison.py
-----------------------------
v1.4 level-5 pre-flight verification.

With --no-comparison only, the v1.4 batch runner produces output
byte-identical to the v1.3.2 baseline at matched seeds on all v1.3
metrics. This is the permanent level-5 regression test added to the
pipeline by v1.4.

The comparison observer is the only addition in v1.4. All other
observers (v1.0 recorder, v1.1 provenance, v1.2 schema, v1.3 family),
the V13Agent, V13World, and the entire run loop are inherited unchanged.
If the v1.4 batch runner produces different v1.3 metrics from the
v1.3.2 baseline when --no-comparison is set, that is a Category α
failure and must be diagnosed before the full v1.4 batch runs.

This script verifies 10 runs at cost=1.0, steps=20000, using seeds
from run_data_v1_3.csv. It compares all v1.3 fields present in both
the v1.4 output and the v1.3 baseline.

Exit 0 = pass (all 10 runs match on all v1.3 fields).
...
from curiosity_agent_v1_4_batch import run_one

BASELINE_PATH      = "run_data_v1_3.csv"
VERIFICATION_COST  = 1.0
VERIFICATION_STEPS = 20000
VERIFICATION_RUNS  = 10

# v1.3 fields to verify — all fields present in run_data_v1_3.csv that
# must be byte-identical in v1.4 with --no-comparison.
# Excludes "arch" (legitimately changes from v1_3 to v1_4).
V1_3_VERIFY_FIELDS = [
...
def main():
    ...
    print("v1.4 Level-5 Pre-Flight Verification")
    print(f"  --no-comparison: v1.4 output must match v1.3.2 baseline")
    print(f"  on all v1.3 metrics at matched seeds.")
    ...
    for run_idx in range(VERIFICATION_RUNS):
        ...
        (metrics, _, _, _, _, _) = run_one(
            ...
            record_schema=True,
            record_family=True,
            record_comparison=False,   # level-5: comparison disabled
            use_v13_world=True,
        )

        run_failures = []
        for field in V1_3_VERIFY_FIELDS:
            v14_val = normalise(metrics.get(field))
            v13_val = normalise(baseline.get(field))
            if v14_val != v13_val:
                run_failures.append(
                    f"  {field}: v1.4={v14_val!r}  v1.3={v13_val!r}"
                )
    ...
    print()
    if not failures:
        print("Level-5 verification PASSED — 10/10 runs match v1.3.2 baseline.")
        print("Category α pre-condition satisfied.")
        print("Proceed to full v1.4 batch.")
        sys.exit(0)
    else:
        print(f"Level-5 verification FAILED — {len(failures)} mismatch(es).")
        print("Category α failure: diagnose before running v1.4 batch.")
        sys.exit(1)
