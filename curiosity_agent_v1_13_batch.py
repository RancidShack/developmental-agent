...
def _yellow_green_complete(agent, pe_obs):
    """Return True if YELLOW and GREEN family objectives are complete.

    'Complete' means both family hazards have been transformed to KNOWLEDGE
    (transition fired) in the current environment. This is the pre-condition
    for the v1.13 transfer gate alongside the predicted record write.
    """
    # Use PE observer's summary_metrics to check resolution
    pe = pe_obs.summary_metrics()
    yellow_done = pe.get("yellow_pre_transition_entries", 0) > 0 and \
                  pe.get("yellow_resolution_window") is not None
    green_done  = pe.get("green_pre_transition_entries", 0) > 0 and \
                  pe.get("green_resolution_window") is not None

    # Alternative: check world.object_type directly for KNOWLEDGE status
    # Both paths are consistent; PE resolution window is the cleaner signal.
    return yellow_done and green_done


def _check_transfer_condition(agent, pe_obs, schema_v13_obs):
    """Return True if ENV1 transfer condition is satisfied.

    Transfer condition (pre-registered §2.3):
      1. YELLOW family complete
      2. GREEN family complete
      3. Predicted schema record written for haz_blue

    This replaces the v1.12 full-environment-completion gate.
    """
    if not _yellow_green_complete(agent, pe_obs):
        return False
...
def _run_environment(
    ...
    # ---- Inject mandatory waypoints ----
    if env_number == 1:
        # haz_blue is a mandatory Phase 1 target in ENV1.
        # Inject its position at the front of the waypoint queue so the
        # agent approaches it before transfer is possible.
    ...
    if env_number == 2 and directed_search_target is not None:
        # att_blue directed search: inject as priority waypoint in ENV2.
    ...
    for step in range(num_steps):
        ...
        if (env_number == 2 and att_blue_mastery_step is None
            ...
            # Check if mastery has been recorded in provenance
            prov_dict = prov_obs.agent.flags if hasattr(prov_obs, 'agent') else {}
            # Simpler: check world object type — attractor becomes KNOWLEDGE on mastery
        ...
        # Transfer is checked but does NOT interrupt the simulation loop.
        # The transfer_condition_met flag is read by run_one() after
        # _run_environment() returns to decide whether to launch ENV2.
        if (env_number == 1 and not transfer_condition_met
                and haz_blue_approached):
            if with_prediction and schema_v13_obs is not None:
                if _check_transfer_condition(agent, pe_obs, schema_v13_obs):
