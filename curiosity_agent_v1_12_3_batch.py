"""
curiosity_agent_v1_12_batch.py
--------------------------------
v1.12 batch runner.

CHANGES FROM V1.11
  - BATCH_STEPS extended to 500,000 (from 320,000). Pre-registered §2.1.
  - env2 activation counter reset: agent.activation_step and
    end_state_banked reset at start of Environment 2. env2_activation_step
    tracked independently. Fixes v1.11 data collection artefact.
  - Q5 structural individuation analysis: pairwise edit distance over
    link-type sequences, written to q5_individuation_v1_12.csv.
  - env2 output fields extended: env2_phase_1_end_step,
    env2_green_resolution_window added.
  - Seed source: run_data_v1_11.csv (primary). Falls back to
    run_data_v1_10_1.csv.

ARCHITECTURE UNCHANGED
  All eleven observers inherited. V111CausalObserver inherited.
  V110Agent, V17World, phase schedule, drive composition unchanged.
  seed_env2 = seed + 10000 inherited.

ADDITIVE DISCIPLINE
  --no-causal:  disables causal observer; Q5 absent; all Q1–Q4 outputs
                byte-identical to v1.11 at matched seeds.
  --no-env2:    skips second-environment architecture.

PRE-FLIGHT
  Level-12 re-run (--no-causal) at 500,000 steps.
  Level-13 re-run at 500,000 steps.

Usage:
    python curiosity_agent_v1_12_batch.py [--no-report]
                                           [--no-goal]
                                           [--no-counterfactual]
                                           [--no-belief-revision]
...
SEED_CSV_PRIMARY   = "run_data_v1_11.csv"
SEED_CSV_FALLBACK  = "run_data_v1_10_1.csv"
RUN_DATA_CSV       = "run_data_v1_12.csv"
PROVENANCE_CSV     = "provenance_v1_12.csv"
GOAL_CSV           = "goal_v1_12.csv"
COUNTERFACTUAL_CSV = "counterfactual_v1_12.csv"
BELIEF_REVISION_CSV = "belief_revision_v1_12.csv"
CAUSAL_CSV         = "causal_v1_12.csv"
REPORT_CSV         = "report_v1_12.csv"
REPORT_SUMMARY_CSV = "report_summary_v1_12.csv"
END_STATE_DRAW_CSV = "end_state_draw_log_v1_12.csv"
ENV2_RUN_DATA_CSV  = "env2_run_data_v1_12.csv"
Q5_INDIVIDUATION_CSV = "q5_individuation_v1_12.csv"
    ...
    with open(path) as f:
        for row in csv.DictReader(f):
            try:
                ...
                # Accept any num_steps — seed source may be v1.11 (320k)
                # or v1.10 fallback; we seed by (cost, run_idx) not steps
                if key not in seeds:
...
def _run_environment(
    ...
    if carry_agent is not None:
        ...
        agent = carry_agent
        agent.world = world
        # Reset per-environment state for Environment 2.
        # activation_step and end_state_banked track the CURRENT
        # environment; they must be reset so env2_activation_step
        # reflects Environment 2 completion, not Environment 1.
        if hasattr(agent, 'end_state_banked'):
            agent.end_state_banked = False
        if hasattr(agent, 'activation_step'):
            agent.activation_step = None
        # Reset phase-completion tracking so phase schedule
        # runs from scratch in the new environment
    ...
    # Note: CF, BR, PE, goal, schema, family, comparison observers are
    # always instantiated fresh per environment (not carried).
    # Only prov_obs and the agent Q-table carry across environments.
    else:
    ...
    if with_counterfactual:
        ...
        # Explicitly reset temporal exclusion state in case the
        # monkey-patch (v1_10_observer_substrates) left class-level
        # state from a prior instantiation.
        cf_obs._records              = []
    ...
    env2_activation_step   = None   # tracked independently for env2
    ...
    for step in range(num_steps):
        ...
        # Track env2 activation independently — capture the first step
        # at which agent.activation_step becomes non-None in this env.
        if env2_activation_step is None:
            a = getattr(agent, 'activation_step', None)
            if a is not None:
                env2_activation_step = step   # step in THIS environment
...
def run_one(run_idx, seed, hazard_cost,
    ...
    # The primary biographical report ALWAYS uses env1's observers.
    # env2 observers are used only for env2_row metrics.
    # The causal observer reads env1's prov_obs (which carries across
    # environments via carry_prov_obs) so the full arc is captured.
    report_env  = env1
...
def _build_env2_row(run_idx, seed, hazard_cost, env2_seed,
    ...
    pe_summary = pe_obs.summary_metrics()
    # Use the independently tracked env2 activation step, not the agent
    # attribute (which reflects the carried Q-table and is unreliable).
    activation_step = env2.get('env2_activation_step')

    # Resolution windows in env2
    yel_win  = pe_summary.get("yellow_resolution_window")
    grn_win  = pe_summary.get("green_resolution_window")

    # Q5 chains from env2 causal records
    ...
    return {
        ...
        "env2_phase_1_end_step":   env2.get("meta", {}).get("phase_1_end_step"),
        "env2_end_state_banked_step": es_step,
        "env2_yellow_pre_transition_entries": pe_summary.get(
            "yellow_pre_transition_entries"),
        "env2_yellow_resolution_window": yel_win,
        "env2_green_pre_transition_entries": pe_summary.get(
...
def _chain_link_sequence(chain_record):
    """Return the ordered tuple of link types for a causal chain."""
    links = chain_record.get("links", [])
    return tuple(lnk["link_type"] for lnk in links)


def compute_q5_individuation(all_causal_records, meta):
    """Compute pairwise Q5 chain structural distances.

    Returns list of row dicts for q5_individuation_v1_12.csv.
    Within-run pairs: chains from the same run_idx.
    Between-run pairs: chains from different run_idx values.

    Chain structural distance: normalised edit distance over
    the ordered link-type sequence of each chain.
    """
    rows = []
    valid = [
        (r.get("run_idx"), r.get("seed"), r.get("hazard_cost"),
         r.get("object_id"), _chain_link_sequence(r))
        for r in all_causal_records
        if r.get("chain_depth", 0) >= CHAIN_DEPTH_MINIMUM
    ]
    ...
    for i in range(len(valid)):
        for j in range(i + 1, len(valid)):
            ri, si, ci, oi, seq_i = valid[i]
            rj, sj, cj, oj, seq_j = valid[j]
            dist = _edit_distance_normalised(list(seq_i), list(seq_j))
            rows.append({
                "arch":                  arch,
                "run_idx_a":             ri,
                "run_idx_b":             rj,
                ...
                "chain_object_a":        oi,
                "chain_object_b":        oj,
                "link_sequence_a":       "|".join(seq_i),
                "link_sequence_b":       "|".join(seq_j),
...
def main():
    ...
    print("v1.12 Batch — Multi-Environment Developmental Transfer")
    ...
    env2_runs            = 0
    incomplete_runs      = []
    all_causal_records   = []   # accumulated for Q5 individuation analysis
    ...
    csv_files = {
        ...
        "q5i":  open(Q5_INDIVIDUATION_CSV,"w", newline=""),
    }
    ...
    writers = {
        ...
        "q5i":  csv.DictWriter(csv_files["q5i"],  fieldnames=Q5_INDIVIDUATION_FIELDS),
    }
    ...
    try:
        ...
        for job_num, (run_idx, seed, hazard_cost) in enumerate(jobs):
            ...
            if with_causal and causal_obs is not None:
                ...
                valid = causal_obs.valid_chains()
                total_causal_chains += len(valid)
                # Accumulate full records (with links) for individuation
                all_causal_records.extend(causal_obs.get_substrate())
        ...
        # -------------------------------------------------------------------
        # Category ζ: Q5 structural individuation (inside try, before close)
        # -------------------------------------------------------------------
        if with_causal and report and all_causal_records:
            print()
            print("Category ζ: Q5 structural individuation")
            valid_chains_q5 = [r for r in all_causal_records
                               if r.get("chain_depth", 0) >= CHAIN_DEPTH_MINIMUM]
            print(f"  Valid chains for analysis: {len(valid_chains_q5)}")

            object_classes = sorted(set(r.get("object_id","") for r in valid_chains_q5))
            print(f"  Object classes with chains: {object_classes}")

            c1_pass = len(object_classes) >= 2
            print(f"  Component 1 (multi-object chains): "
                  f"{'PASS' if c1_pass else 'FAIL — single object class only'}")

            if valid_chains_q5:
                indiv_meta = {"arch": ARCH}
                indiv_rows = compute_q5_individuation(all_causal_records, indiv_meta)
                ...
                if between:
                    ...
                    c2_pass = mean_b > mean_w
                    print(f"  Component 2 (between > within): "
                          f"{'PASS' if c2_pass else 'FAIL'} "
                          f"between={mean_b:.4f} within={mean_w:.4f}")
                else:
                    print("  Component 2: insufficient pairs for comparison")

                p1_counts = {}
                for r in valid_chains_q5:
                    oid = r.get("object_id","")
                    has_p1 = r.get("links_phase1_absence", 0) > 0
                    p1_counts.setdefault(oid, []).append(has_p1)
                print("  Component 3 (phase1_absence variability by object):")
                for oid, vals in sorted(p1_counts.items()):
                    n_present = sum(vals)
                    print(f"    {oid}: {n_present}/{len(vals)} chains have "
                          f"phase1_absence link")
    ...
    if with_env2:
        ...
        if env2_runs < 8:
            ...
            print(f"  Component 1 (yellow eliminated): see env2_run_data_v1_12.csv")
            print(f"  Component 2 (env2 activation < env1): "
                  f"see env2_activation_step vs run_data activation_step")
            print(f"  Component 3 (green family transfer): "
                  f"see env2_green_resolution_window")
