"""
curiosity_agent_v1_16_batch.py
----------------------------------
v1.16: Purpose Without Direction.

Three additions to the v1.14.1 reparative return architecture:

  1. NO END_STATE
     has_end_state=False throughout ENV1. The agent completes all
     completable work and receives no completion signal. Post-completion
     behaviour in return ENV1 is the primary empirical finding.

  2. HAZ_GREY — one permanently unreachable hazard
     PYRAMID_3D form; no attractor precondition in any environment.
     Added to ENV1 via V16_HAZ_GREY_FAMILY. Excluded from transfer gate
     via unreachable_hazards=['haz_blue','haz_grey']. Every encounter
     produces cost; the record is permanently unresolvable.
     Tracked in unreachable_v1_16.csv.

  3. SHAPE CONNECTOR — second basis_chain on att_blue prediction
     When both GREEN (SPHERE_3D↔CIRCLE_2D) and YELLOW (PYRAMID_3D↔
     TRIANGLE_2D) family chains are complete and haz_blue (CUBE_3D)
     has been approached, the shape connector adds shape-prefixed basis
     entries alongside the colour basis entries. basis_chains becomes:
     ['haz_yellow','haz_green','shape:haz_green','shape:haz_yellow']
     Both connectors converge on predicted_precondition='att_blue'
     (SQUARE_2D) via independent reasoning axes.

WORLD FORM GRAMMAR (v1.16 corrected)
  SPHERE_3D ↔ CIRCLE_2D  — GREEN family  (att_green corrected v1.16)
  PYRAMID_3D ↔ TRIANGLE_2D — YELLOW family  (unchanged)
  CUBE_3D ↔ SQUARE_2D   — BLUE family   (haz_blue + att_blue corrected v1.16)

POST-COMPLETION TRACKING
  After haz_blue→KNOWLEDGE in return ENV1, the agent continues for
  the remaining budget. post_completion_v1_16.csv records:
  steps remaining, contact count, objects contacted, dominant
  behaviour category (attractor_revisit, knowledge_revisit,
  exploration, idle).

ARCH = v1_16. Seeds from run_data_v1_15_3.csv.
PRE-REGISTRATION: v1_16_pre_registration.md (8 May 2026).
"""
import argparse
import csv
import os
...
from v1_13_world import (
    ...
    PERCEPTION_RADIUS,
    GREEN, YELLOW, BLUE,
    SQUARE_2D, TRIANGLE_2D, CIRCLE_2D, SPHERE_3D, PYRAMID_3D, CUBE_3D,
)

# v1.16: haz_grey standalone family (outside GREEN/YELLOW/BLUE grammar)
GREY = "GREY"
HAZ_GREY_ID  = "haz_grey"
HAZ_GREY_POS = (4.0, 3.0, 6.0)   # distinct from all family positions

V16_HAZ_GREY_FAMILY = {
    "colour":   GREY,
    "dist_id":  None, "dist_pos": None,
    "att_id":   None, "att_pos":  None, "att_form": None,
    "haz_id":   HAZ_GREY_ID,
    "haz_pos":  HAZ_GREY_POS,
    "haz_form": PYRAMID_3D,        # shape connector predicts TRIANGLE_2D precondition
}

# ENV1 for v1.16: existing families + haz_grey standalone
ENV1_FAMILIES_V16 = list(ENV1_FAMILIES) + [V16_HAZ_GREY_FAMILY]
...
SEED_CSV_PRIMARY     = "run_data_v1_15_3.csv"
RUN_DATA_CSV         = "run_data_v1_16.csv"
PROVENANCE_CSV       = "provenance_v1_16.csv"
GOAL_CSV             = "goal_v1_16.csv"
COUNTERFACTUAL_CSV   = "counterfactual_v1_16.csv"
BELIEF_REVISION_CSV  = "belief_revision_v1_16.csv"
CAUSAL_CSV           = "causal_v1_16.csv"
REPORT_CSV           = "report_v1_16.csv"
REPORT_SUMMARY_CSV   = "report_summary_v1_16.csv"
END_STATE_DRAW_CSV   = "end_state_draw_log_v1_16.csv"
ENV2_RUN_DATA_CSV    = "env2_run_data_v1_16.csv"
Q5_INDIVIDUATION_CSV = "q5_individuation_v1_16.csv"
PREDICTED_SCHEMA_CSV = "predicted_schema_v1_16.csv"
ARC_COMPLETE_CSV     = "arc_complete_v1_16.csv"
POST_COMPLETION_CSV  = "post_completion_v1_16.csv"   # v1.16 new
UNREACHABLE_CSV      = "unreachable_v1_16.csv"         # v1.16 new        # v1.14 new
...
POST_COMPLETION_FIELDS = [          # v1.16 new
    "arch", "run_idx", "seed", "hazard_cost",
    "arc_complete",
    "transformation_step",
    "post_completion_steps",
    "post_completion_contacts",
    "post_completion_objects",
    "behaviour_category",           # attractor_revisit|knowledge_revisit|exploration|idle
    "shape_connector_fired",
]

UNREACHABLE_FIELDS = [              # v1.16 new
    "arch", "run_idx", "seed", "hazard_cost",
    "haz_grey_approach_count",
    "haz_grey_contact_count",
    "haz_grey_first_approach_step",
    "haz_grey_first_contact_step",
    "unresolvable",
    "shape_connector_fired",
]
...
def _run_environment(
    ...
    unreachable = ['haz_blue', HAZ_GREY_ID] if env_number == 1 else None
    world = V113World(
        families=families,
        hazard_cost=hazard_cost,
        has_end_state=False,           # v1.16: no end_state signal
        seed=seed,
    ...
    # v1.16 new tracking variables
    haz_grey_approach_count     = 0
    haz_grey_contact_count      = 0
    haz_grey_first_approach_step = None
    haz_grey_first_contact_step  = None
    shape_connector_fired        = False
    ...
    for step in range(num_steps):
        ...
        if (with_completion_signal and isinstance(agent, V110Agent)
                and world.end_state_cell is not None):   # v1.16: guard for has_end_state=False
        ...
        # haz_grey approach and contact tracking (ENV1 only, v1.16)
        if env_number == 1:
            hgp = world.object_positions.get(HAZ_GREY_ID)
            if hgp is not None:
                import math as _mh
                dg = _mh.sqrt(sum((a-b)**2 for a,b in
                                   zip(world.agent_pos, hgp)))
                if dg <= PERCEPTION_RADIUS:
                    haz_grey_approach_count += 1
                    if haz_grey_first_approach_step is None:
                        haz_grey_first_approach_step = step
                if contact_oid == HAZ_GREY_ID:
                    haz_grey_contact_count += 1
                    if haz_grey_first_contact_step is None:
                        haz_grey_first_contact_step = step
        ...
        if (env_number == 1 and haz_blue_approached
            ...
            if n >= PREDICTION_FAMILY_MINIMUM:
                colour_basis = sorted(
                    ...
                    if world.knowledge_unlocked.get(h, False))
                # v1.16 shape connector: second basis_chain from form grammar
                # GREEN (SPHERE_3D↔CIRCLE_2D) + YELLOW (PYRAMID_3D↔TRIANGLE_2D)
                # → CUBE_3D↔SQUARE_2D predicts att_blue = SQUARE_2D
                shape_basis = [f'shape:{h}' for h in colour_basis]
                combined_basis = colour_basis + shape_basis
                schema_v13_obs.add_predicted_record({
                    "object_id":              "haz_blue",
                    "predicted_precondition": "att_blue",
                    "basis_chains":           combined_basis,
                    "prediction_step":        step,
                    "prediction_env":         env_number,
                })
                predicted_record_written = True
                shape_connector_fired = True
    ...
    return dict(
        ...
        world=world,
        # v1.16 new
        haz_grey_approach_count=haz_grey_approach_count,
        haz_grey_contact_count=haz_grey_contact_count,
        haz_grey_first_approach_step=haz_grey_first_approach_step,
        haz_grey_first_contact_step=haz_grey_first_contact_step,
        shape_connector_fired=shape_connector_fired,
    )
...
def _run_return_env1(run_idx, seed, hazard_cost,
                     ...
                     env2_actual_steps=None,
                     shape_connector_fired_in=False):   # v1.16
    ...
    # Step loop — continues after arc_complete for post-completion tracking.
    state               = return_world.observe()
    transformation_step = None
    arc_complete        = False
    arc_timeout         = False
    actual_steps        = RETURN_ENV1_STEPS
    # v1.16: post-completion tracking initialised before loop
    post_completion_steps    = 0
    post_completion_contacts = 0
    post_completion_objects  = []
    _attractor_revisit       = False
    _knowledge_revisit       = False
    _exploration             = False
    ...
    for step in range(RETURN_ENV1_STEPS):
        ...
        # v1.16: do NOT break — continue stepping for post-completion.
        if (not arc_complete and
                return_world.knowledge_unlocked.get('haz_blue', False)):
            transformation_step = step
            arc_complete        = True

        # v1.16: track post-completion contacts inline (no double-stepping)
        if arc_complete and step > transformation_step:
            post_completion_steps += 1
            if contact_oid is not None:
                post_completion_contacts += 1
                if contact_oid not in post_completion_objects:
                    post_completion_objects.append(contact_oid)
                _otype = return_world.object_type.get(contact_oid)
                if _otype == ATTRACTOR:
                    _attractor_revisit = True
                elif _otype == KNOWLEDGE:
                    _knowledge_revisit = True
                else:
                    _exploration = True
    ...
    # Categorise dominant post-completion behaviour
    behaviour_category = "idle"
    if arc_complete:
        if _attractor_revisit:
            behaviour_category = "attractor_revisit"
        elif _knowledge_revisit:
            behaviour_category = "knowledge_revisit"
        elif _exploration:
            behaviour_category = "exploration"

    if not arc_complete and not arc_timeout:
        arc_timeout = True

    arc_total_steps = None
    if arc_complete:
        # FIX 1: additive across environments.
        # transfer_step   = ENV1 steps to transfer gate
        # env2_actual_steps = steps run in ENV2
        # transformation_step = steps in return ENV1 to arc_complete
        arc_total_steps = (
            (transfer_step or 0)
            + (env2_actual_steps or 0)
            + transformation_step
        )
    ...
    post_completion_row = {
        ...
        "arc_complete":           arc_complete,
        "transformation_step":    transformation_step,
        "post_completion_steps":  post_completion_steps,
        "post_completion_contacts": post_completion_contacts,
        "post_completion_objects":  "|".join(post_completion_objects),
        "behaviour_category":     behaviour_category,
        "shape_connector_fired":  shape_connector_fired_in,
    }
    ...
    return dict(
        ...
        post_completion_row=post_completion_row,      # v1.16 new
    )
...
def run_one(run_idx, seed, hazard_cost,
    ...
    env1 = _run_environment(
        ...
        families=ENV1_FAMILIES_V16,    # v1.16: includes haz_grey
    ...
    # FIX 2: if ENV1 wrote a predicted record but exhausted its budget
    # without meeting the transfer condition, the record would remain
    # permanently in 'predicted' state (mark_unresolvable only fired
    # inside ENV2). Mark it unresolvable here before ENV2 is attempted.
    if (with_prediction and schema_v13_obs is not None
            and env1['predicted_record_written']
            and not env1_transfer):
        schema_v13_obs.mark_unresolvable("haz_blue")
    ...
    if with_return and env2 is not None:
        if _return_to_env1_condition_met(schema_v13_obs):
            ...
            return_result = _run_return_env1(
                ...
                shape_connector_fired_in=env1.get('shape_connector_fired', False),  # v1.16
            )
    ...
    return (run_row, statements, summary,
            ...
            arc_complete_row,
            return_result,                                   # v1.16
            env1.get('haz_grey_approach_count', 0),          # v1.16
            env1.get('haz_grey_contact_count', 0),           # v1.16
            env1.get('haz_grey_first_approach_step'),        # v1.16
            env1.get('haz_grey_first_contact_step'),         # v1.16
            env1.get('shape_connector_fired', False))        # v1.16
...
def main():
    ...
    try:
        for key, path in [
            ...
            ('post', POST_COMPLETION_CSV),   # v1.16
            ('unrch', UNREACHABLE_CSV),      # v1.16
        ]:
        ...
        writers['post'] = csv.DictWriter(csv_files['post'], fieldnames=POST_COMPLETION_FIELDS)   # v1.16
        writers['unrch']= csv.DictWriter(csv_files['unrch'],fieldnames=UNREACHABLE_FIELDS)       # v1.16
        ...
        print(f"v1.16 batch: {len(jobs)} runs | "
        ...
        for run_idx, seed, hazard_cost in jobs:
            ...
            (run_row, statements, summary,
             ...
             arc_complete_row, return_result,
             _env1_grey_approaches, _env1_grey_contacts,
             _env1_grey_first_approach, _env1_grey_first_contact,
             _shape_connector_fired) = result
            ...
            # v1.16: post_completion and unreachable rows
            if return_result is not None:
                pc_row = return_result.get('post_completion_row')
                if pc_row is not None:
                    writers['post'].writerow(
                        {k: pc_row.get(k, "") for k in POST_COMPLETION_FIELDS}
                    )
            env1_result = result[3] if len(result) > 3 else None  # env1 data from run_one
            writers['unrch'].writerow({
                "arch":                      ARCH,
                "run_idx":                   run_idx,
                "seed":                      seed,
                "hazard_cost":               hazard_cost,
                "haz_grey_approach_count":   _env1_grey_approaches,
                "haz_grey_contact_count":    _env1_grey_contacts,
                "haz_grey_first_approach_step": _env1_grey_first_approach,
                "haz_grey_first_contact_step":  _env1_grey_first_contact,
                "unresolvable":              True,
                "shape_connector_fired":     _shape_connector_fired,
            })
