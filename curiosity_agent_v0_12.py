"""
curiosity_agent_v0_12.py
------------------------
Developmental agent in a 20x20 structured environment — v0.12
extension of v0.11.2 with category-level generalisation in the
threat layer via cell-type signature-matching.

v0.10 introduced experience-driven persistent threat representation:
each hazard cell required three entries to convert its flag, with
no mechanism for transferring flag status across cells. The v0.10
paper characterised a boundary condition at cost 5.0 at 20,000 steps
in which late-encountered hazards could not complete the three-entry
conversion within Phase 3's time budget, producing 5/10 Phase 3
cleanness against the otherwise-uniform 8-10/10 across the matrix.

v0.12 modifies the flag-conversion rule to address this boundary
condition. When the agent enters a hazard cell, the architecture
first checks whether any other hazard cell has threat_flag = 1.
If yes, the entered cell's threat_flag is set to 1 immediately on
this entry. If no, the standard three-entry conversion rule applies.

The signature is cell type alone: HAZARD cells are treated as members
of the same category for the purposes of flag transfer. The first
hazard cell still requires three entries (because no other hazard
is flagged at the time of its first three encounters); every
subsequent novel hazard cell encountered is flagged on first entry.

This is signature-matching with first-entry flagging on signature
match. It is not one-shot full generalisation (the agent must still
encounter cells to flag them) and not graded probabilistic prior.
The architectural change is contained entirely within
update_threat_layer.

The Category F honesty constraint from the pre-registration: the
architecture already represents cell types directly through the
hazard_cells set. Whether v0.12 success demonstrates "category-level
generalisation works in this architecture" or "the architecture's
existing type representation was sufficient once the rule allowed
it to act" cannot be distinguished from this iteration alone.
Richer-signature work (adjacency patterns, environmental markers)
is reserved for v0.14+.

See v0.12-preregistration.md for full reasoning, including the
stopping rule named in Section 10 of that document.

All other architectural elements from v0.11.2 (which inherits
v0.11.1, v0.11, and v0.10) are retained unchanged.

Run from Terminal with:
    python3 curiosity_agent_v0_12.py
"""
...
def run():
    ...
    report_filename = f"self_report_v0_12_c{HAZARD_COST}.txt"
    ...
    fig.suptitle(
        f"v0.12  —  cost={HAZARD_COST}   mastery_thresh={MASTERY_THRESHOLD}   "
        f"mastery_bonus={MASTERY_BONUS}",
        fontsize=13, y=0.995
    )
    plt.tight_layout(rect=[0, 0, 1, 0.98])

    plot_filename = f"run_output_v0_12_c{HAZARD_COST}.png"
