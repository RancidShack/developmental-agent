...
verify_v1_15_3.py
-----------------
Pre-flight verifier for curiosity_agent_v1_15_3_batch.py.
...
Criteria:
  15.1.1  Social observation gate — fires only when all three conditions
          ...
          v1.15.3: waypoints injected at att_pos on fire; verified explicitly.
  ...
  15.1.3  Q6 statement produced for socially_corroborated record;
          ...
          SimpleNamespace objects; getattr access verified.
...
try:
    ...
    from curiosity_agent_v1_15_3_batch import (
        ...
        SOCIAL_PERCEPTION_RADIUS, SOCIAL_OBS_WAYPOINT_COUNT,
...
def criterion_15_1_1():
    ...
    v1.15.3: verifies waypoints are injected into the world on fire.
    """
    if not _IMPORTS_OK:
    ...
    # v1.15.3: fire and verify waypoints are injected
    waypoints_before = len(getattr(world, '_waypoints', []) or [])
    _fire_social_observation(ctx_b, ctx_a, world, step=100, env_number=2)
    waypoints_after = len(getattr(world, '_waypoints', []) or [])
    waypoints_injected = waypoints_after - waypoints_before
    correct_waypoint_count = waypoints_injected == SOCIAL_OBS_WAYPOINT_COUNT

    # v1.15.2: B far from att_blue but within SOCIAL_PERCEPTION_RADIUS — should fire
    ctx_b2 = _make_ctx('b', _make_agent(world, 99), 99, world)
    ctx_b2.pos = (att_pos[0], att_pos[1], max(0.0, att_pos[2] - SOCIAL_PERCEPTION_RADIUS * 0.8))
    fires_extended_range = _check_social_observation(ctx_b2, ctx_a, world, 2)

    # A NOT at att_blue → should not fire
    ctx_b2.social_observation_fired = False
    ctx_a.pos = haz_pos
    ctx_b2.pos = tuple(p + PERCEPTION_RADIUS * 0.5 for p in haz_pos)
    no_fire_a_not_at_att = not _check_social_observation(ctx_b2, ctx_a, world, 2)

    # B beyond SOCIAL_PERCEPTION_RADIUS → should not fire
    ctx_a.pos = att_pos
    ctx_b2.pos = (0.0, 0.0, 0.0)
    no_fire_b_too_far = not _check_social_observation(ctx_b2, ctx_a, world, 2)

    # ENV1 → should not fire
    ctx_b2.pos = tuple(p + PERCEPTION_RADIUS * 0.5 for p in att_pos)
    no_fire_wrong_env = not _check_social_observation(ctx_b2, ctx_a, world, 1)

    # Already fired → should not fire again
    ctx_b2.social_observation_fired = True
    no_fire_already = not _check_social_observation(ctx_b2, ctx_a, world, 2)

    checks = {
        "fires when all conditions met":                    fires_when_all_met,
        f"waypoints injected = SOCIAL_OBS_WAYPOINT_COUNT ({SOCIAL_OBS_WAYPOINT_COUNT})": correct_waypoint_count,
    ...
    ok = not failed
    detail = (f"waypoints_injected={waypoints_injected} "
              f"(expected {SOCIAL_OBS_WAYPOINT_COUNT})"
...
def main():
    ...
    print("v1.15.3 Pre-flight Verifier")
    print("=" * 50)
    print()

    criteria = [
        ("15.1.1  Social observation gate (waypoint injection verified)",
         criterion_15_1_1),
    ...
    print(f"v1.15.3: {n_pass}/{len(results)} PASS, {n_fail} FAIL")
    ...
    if n_fail > 0:
        ...
        print()
        print("All v1.15.3 criteria PASS. Full batch authorised.")
        sys.exit(0)
