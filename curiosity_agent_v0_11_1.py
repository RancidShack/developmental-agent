"""
curiosity_agent_v0_11_1.py
--------------------------
Developmental agent in a 20x20 structured environment — v0.11.1
amendment to v0.11, applying preference reset at the mastery step so
that mastered attractors lose all signal that would draw the agent
back to them.

v0.11 introduced attractor depletion (feature_reward goes to zero
post-mastery) and primitive attraction bias clearing for mastered
attractors. v0.11's single-run probe revealed that the cell_preference
accumulator was not affected by mastery: pre-mastery visits accumulated
preference from the per-visit feature reward (0.15 x N=3 = 0.45) plus
the MASTERY_BONUS (1.0) at the banking step, depositing approximately
1.45 units of preference mass. This persisted after mastery and
dominated Phase 3 action selection via preference_weight = 0.8.

v0.11.1 adds the missing piece: at the moment of mastery, the
attractor's accumulated cell_preference is reset to zero. Combined
with the v0.11 changes (depleted feature reward, cleared attraction
bias), this means a banked attractor produces no architectural signal
pulling the agent back. The cell remains visitable but no mechanism
reinforces returning to it.

The reset is applied INSIDE update_model, AFTER the standard preference
accumulation, so that the v0.11 sequence (counter incremented, bonus
delivered, preference accumulated) completes before the reset zeroes
the cell. Net effect: cell_preference at the banking step is zero,
not 1.45. The MASTERY_BONUS still influences Q-value updates via the
intrinsic reward, just not via long-term preference.

See v0.11.1-preregistration-amendment.md for full reasoning.

All other architectural elements from v0.11 (which inherits v0.10) are
retained unchanged.

Run from Terminal with:
    python3 curiosity_agent_v0_11_1.py
"""
...
class DevelopmentalAgent:
    """v0.11.1 amendment to v0.11. Adds one architectural change:
    cell_preference is reset to zero at the moment of mastery, so that
    mastered attractors do not retain accumulated preference mass that
    would draw the agent back to them in Phase 3.
    ...
    v0.11.1 additions:
      - update_model resets cell_preference[cell] to 0.0 at the banking
        step, AFTER the standard preference accumulation completes.
        Net effect: cell_preference at the banking step ends at zero.
    """
    ...
    def update_model(self, state, action, next_state, error, r_progress, r_feature):
        ...
        # its preference to zero. This happens AFTER the standard
        # accumulation so the sequence (counter increment, bonus
        # delivery via feature_reward, preference accumulation) all
        # complete before the reset takes effect.
        # The banking step is identified by: cell is in mastery_flag,
        # flag is set to 1, AND visit counter equals MASTERY_THRESHOLD
        # exactly (not greater — that would be a post-mastery visit).
...
def run():
    ...
    lines = []
    lines.append("=" * 72)
    lines.append(f"v0.11.1 CHARACTERISATION")
    ...
    report_filename = f"self_report_v0_11_1_c{HAZARD_COST}.txt"
    ...
    fig.suptitle(
        f"v0.11.1  —  cost={HAZARD_COST}   mastery_thresh={MASTERY_THRESHOLD}   "
        f"mastery_bonus={MASTERY_BONUS}",
        fontsize=13, y=0.995
    )
    plt.tight_layout(rect=[0, 0, 1, 0.98])

    plot_filename = f"run_output_v0_11_1_c{HAZARD_COST}.png"
