"""
curiosity_agent_v1_14_1_batch.py
----------------------------------
v1.14.1 patch: two data-integrity fixes on v1.14. No architectural changes.

FIXES FROM V1.14
  FIX 1 — arc_total_steps: subtracted env1_surprise_step from
    transformation_step, producing large negative values (indices
    from different environments are not in the same space). Corrected
    formula: transfer_step + env2_actual_steps + transformation_step.
    env2_actual_steps added as parameter to _run_return_env1.

  FIX 2 — orphan predicted record: ENV1 could write a predicted record
    then exhaust its budget without transferring, leaving the record
    permanently in 'predicted' state. Fixed: mark_unresolvable fires
    in run_one when ENV1 exits without transfer and a predicted record
    exists and was written.

All v1.14 core results unchanged: 29/32 arc_complete, 0 arc_timeout,
0 hallucinations, 42/43 depth-6 causal chains.

PRE-REGISTRATION: v1_14_pre_registration.md (8 May 2026).
Seed source: run_data_v1_13_8.csv (same seeds as v1.14).
"""
import argparse
import csv
import os
...
RUN_DATA_CSV         = "run_data_v1_14_1.csv"
PROVENANCE_CSV       = "provenance_v1_14_1.csv"
GOAL_CSV             = "goal_v1_14_1.csv"
COUNTERFACTUAL_CSV   = "counterfactual_v1_14_1.csv"
BELIEF_REVISION_CSV  = "belief_revision_v1_14_1.csv"
CAUSAL_CSV           = "causal_v1_14_1.csv"
REPORT_CSV           = "report_v1_14_1.csv"
REPORT_SUMMARY_CSV   = "report_summary_v1_14_1.csv"
END_STATE_DRAW_CSV   = "end_state_draw_log_v1_14_1.csv"
ENV2_RUN_DATA_CSV    = "env2_run_data_v1_14_1.csv"
Q5_INDIVIDUATION_CSV = "q5_individuation_v1_14_1.csv"
PREDICTED_SCHEMA_CSV = "predicted_schema_v1_14_1.csv"
ARC_COMPLETE_CSV     = "arc_complete_v1_14_1.csv"        # v1.14 new
    ...
    if with_return and env2 is not None:
        if _return_to_env1_condition_met(schema_v13_obs):
            ...
            return_result = _run_return_env1(
                ...
                env2_actual_steps=env2.get('actual_steps'),   # FIX 1
            )
