...
def main():
    ...
    if with_causal and report and all_causal_records:
        ...
        valid_chains = [r for r in all_causal_records
                        if r.get("chain_depth", 0) >= CHAIN_DEPTH_MINIMUM]
        print(f"  Valid chains for analysis: {len(valid_chains)}")

        object_classes = sorted(set(r.get("object_id","") for r in valid_chains))
        ...
        if valid_chains:
            ...
            for row in indiv_rows:
                writers["q5i"].writerow(
                    {k: row.get(k, "") for k in Q5_INDIVIDUATION_FIELDS}
                )
            ...
            # Phase 1 absence variability
            p1_counts = {}
            for r in valid_chains:
                oid = r.get("object_id","")
