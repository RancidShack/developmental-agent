"""
curiosity_agent_v1_4_batch.py
------------------------------
v1.4 cross-family comparison batch runner. Inherits the v1.3.2
architecture unchanged; runs five parallel observers:
  1. v1.0 recorder           (V1Recorder)
  2. v1.1 provenance store   (V1ProvenanceStore)
  3. v1.2 schema observer    (V13SchemaObserver — 7 cell types)
  4. v1.3 family observer    (V13FamilyObserver)
  5. v1.4 comparison observer (V14ComparisonObserver)  ← new this iteration

Outputs:
  run_data_v1_4.csv              per-run metrics: all v1.3 fields
                                  retained plus v1.4 comparison summary
                                  fields.
  provenance_v1_4.csv            per-flag formation records (v1.1 schema).
  snapshots_v1_4.csv             v1.1 provenance snapshots.
  schema_v1_4.csv                per-run schema content (7 cell types).
  family_v1_4.csv                per-run family record (v1.3 schema).
  comparison_v1_4.csv            per-run cross-family comparison record.
                                  ← new output file this iteration
  snapshots_v1_0_under_v1_4.csv v1.0 snapshot file for continuity.

The v0.14 agent (via V14Agent subclass) is used unchanged. V13World is
...
Flag combinations (pre-flight verification levels):
  --no-instrument --no-provenance --no-schema --no-family --no-comparison:
  ...
  --no-schema --no-family --no-comparison:
      matches v1.1 baseline (level 3).
  --no-family --no-comparison:
  ...
  --no-comparison only:
      ...
      metrics. This is the permanent level-5 regression test added to
      the pipeline by v1.4. See verify_v1_4_no_comparison.py.
  (defaults):
      full v1.4 production run with all five observers.

Seeds: matched to v1.3.2 batch via run_data_v1_3.csv.

Run from Terminal with:
    python3 curiosity_agent_v1_4_batch.py
"""
...
def run_one(hazard_cost, num_steps, seed, run_idx,
            ...
            use_v13_world=True):
    """Execute one v1.4 run.

    use_v13_world controls whether V13World (with colour cells) or
    the v0.14 StructuredGridWorld (via V12World shim) is used.
    For regression testing, use_v13_world=False ensures the environment
    is identical to earlier baselines.

    record_comparison controls whether V14ComparisonObserver is
    instantiated. With record_comparison=False, output is byte-identical
    to v1.3.2 baseline at matched seeds (level-5 regression).
    The comparison observer requires record_family=True; if
    record_family=False, record_comparison is treated as False.

    All five False + use_v13_world False:
    ...
    All five True + use_v13_world True:
        full v1.4 production run.
    """
    np.random.seed(seed)
    ...
    agent = V14Agent(world, num_steps)
    ...
    # The comparison observer requires the family observer.
    # If family observer is not active, comparison is suppressed.
    comparison_obs = None
    ...
    # ------------------------------------------------------------------
    # Metrics assembly — all v1.3 fields retained, v1.4 fields appended.
    # ------------------------------------------------------------------
    metrics = {
        "arch":        "v1_4",
        "hazard_cost": hazard_cost,
    ...
    return metrics, recorder, provenance, schema_obs, family_obs, comparison_obs
...
def main():
    ...
    ap.add_argument("--out",              default="run_data_v1_4.csv")
    ap.add_argument("--snapshots-out",    default="snapshots_v1_4.csv")
    ap.add_argument("--provenance-out",   default="provenance_v1_4.csv")
    ap.add_argument("--schema-out",       default="schema_v1_4.csv")
    ap.add_argument("--family-out",       default="family_v1_4.csv")
    ap.add_argument("--comparison-out",   default="comparison_v1_4.csv")
    ap.add_argument("--v10-snapshots-out",
                    default="snapshots_v1_0_under_v1_4.csv")
    ap.add_argument("--baseline",         default="run_data_v1_3.csv")
    ...
    ap.add_argument("--no-family",       action="store_true",
                    help=("Disable v1.3 family observer and use v1.2-identical "
                          "world. Level-4 regression test against v1.2 baseline."))
    ap.add_argument("--no-comparison",   action="store_true",
                    help=("Disable v1.4 comparison observer. With all other "
                          "observers active and V13World, output is byte-identical "
                          "to v1.3.2 baseline at matched seeds. This is the "
                          "permanent level-5 regression test added by v1.4."))
    ...
    for r in baseline:
        if r.get("arch") not in (
            "v1_3", "v1_2", "v1_1", "v1_0", "v0_14", "v0_14_repl"
        ):
            continue
        try:
            ...
            seed = int(r["seed"])
            priority = {
                "v1_3": 0, "v1_2": 1, "v1_1": 2,
                "v1_0": 3, "v0_14": 4, "v0_14_repl": 5,
            }
    ...
    print(f"v1.4 CROSS-FAMILY COMPARISON BATCH")
    print(f"  Architecture:         v0.14 (unchanged) + V13World + V14Agent")
    ...
    for i, (cost, steps, run_idx, seed) in enumerate(jobs):
        ...
        (metrics, recorder, provenance, schema_obs,
         family_obs, comparison_obs) = run_one(
        ...
        # Comparison status for progress line
        m3 = metrics.get("traversal_sequence_distance")
        m3_str = f"{m3:.3f}" if m3 is not None else "---"
        interleave = metrics.get("traversal_interleaving") or "-"
        interleave_short = {
            "green_then_yellow": "G>Y",
            "yellow_then_green": "Y>G",
            "interleaved":       "mix",
        }.get(interleave, "-")
        ...
        print(
            ...
            f"d={m3_str} [{interleave_short}] "
            f"({elapsed_run:.1f}s, ETA {eta/60:.1f}m)"
        )

    elapsed = time.time() - t_start
    print(f"\nv1.4 batch complete in {elapsed/60:.1f} minutes.")
    if instrument:
    ...
    if record_comparison:
        print(f"Both families complete:     {both_complete_count} of {len(jobs)}")
        print(f"Comparison complete:        {comparison_complete_count} of {len(jobs)}")

    # ------------------------------------------------------------------
    # Write per-run CSV — all v1.3 fields retained, v1.4 fields appended.
    # ------------------------------------------------------------------
    fieldnames = [
