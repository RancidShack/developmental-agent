...
verify_v1_15_1.py
-----------------
Pre-flight verifier for curiosity_agent_v1_15_1_batch.py.
...
Criteria:
  15.1.1  Social observation gate — fires only when all three conditions
          ...
          CONTACT_RADIUS of att_blue, Agent B within PERCEPTION_RADIUS
...
try:
    ...
    from curiosity_agent_v1_15_1_batch import (
...
def criterion_15_1_1():
    ...
    # Place A at att_blue, B within PERCEPTION_RADIUS of A → should fire
    ctx_a.pos = att_pos
    ...
    # A at att_blue, B too far away → should not fire
    ctx_a.pos = att_pos
    ctx_b.pos = (0.0, 0.0, 0.0)  # far corner
    ...
    checks = {
        ...
        "no fire: B too far from A":        no_fire_b_too_far,
...
def criterion_15_1_3():
    ...
    checks = {
        ...
        "Q6 source_resolves=True (no hallucination)":  all(s["source_resolves"] for s in q6_soc),
        "Q6 query_type is social_corroboration":       q6_soc[0]["query_type"] == "social_corroboration" if q6_soc else False,
        "Q6 mentions own mastery":                     "own mastery" in q6_soc[0]["text"].lower() if q6_soc else False,
    }
    failed = [k for k, v in checks.items() if not v]
    ok = not failed
...
def main():
    ...
    print("v1.15.1 Pre-flight Verifier")
    print("=" * 50)
    print()

    criteria = [
        ("15.1.1  Social observation gate (three conditions, no false fires)",
         criterion_15_1_1),
        ("15.1.2  Schema state transitions (predicted→corroborated→confirmed)",
         criterion_15_1_2),
        ("15.1.3  Q6 produced for corroborated; not for plain confirmed; no hallucination",
         criterion_15_1_3),
    ...
    print(f"v1.15.1: {n_pass}/{len(results)} PASS, {n_fail} FAIL")
    ...
    if n_fail > 0:
        ...
        print()
        print("All v1.15.1 criteria PASS. Full batch authorised.")
        sys.exit(0)
